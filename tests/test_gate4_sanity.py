...
OFFICIAL_JSON = "data/tracking/latest/match_tracking_data_official.json"

with open(OFFICIAL_JSON) as f:
...
print("=== GATE 4: TEAM LABEL COVERAGE (re-check) ===")
...
ideal = sum(c for (b, w, g), c in per_frame_combos.items() if b >= 3 and w >= 3)
total_frames = sum(per_frame_combos.values())
pct = 100 * ideal / total_frames

print(f"\n  Frames with >=3 blue AND >=3 white: {ideal}/{total_frames} ({pct:.1f}%)")
print(f"  [{'PASS' if pct > 50 else 'FAIL'}]")